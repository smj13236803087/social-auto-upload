...
Rules:
- Shorts only (default channel @grillnation/shorts)
- 1 short per run (schedule twice daily = 2/day)
- Highest quality download (bv*+ba)
- Skip shorts already uploaded to Douyin
- Chinese copy from grill_food_copy_library.json in order
- Delete local files after successful Douyin upload
"""
...
DEFAULT_CHANNEL = "https://www.youtube.com/@grillnation/shorts"
DEFAULT_ACCOUNT = "boss rabbit"
DEFAULT_STATE = ROOT / "data" / "yt_douyin_sync_state.json"
DEFAULT_INBOX = ROOT / "videos" / "yt_douyin_inbox"
DEFAULT_COPY_LIBRARY = ROOT / "data" / "grill_food_copy_library.json"
DEFAULT_COOKIES_FROM_BROWSER = "edge"
DEFAULT_DY_TAGS = "美食,烧烤,烤肉"
...
def download_highest(
    ...
    yt_dlp = shutil.which("yt-dlp") or "yt-dlp"
    out_tmpl = str(inbox / f"dy_%(upload_date)s_%(id)s_%(title).80B.%(ext)s")
    _run(
        [
            yt_dlp,
            "--no-update",
...
def main() -> int:
    ...
    parser.add_argument("--tags", default=DEFAULT_DY_TAGS, help="Douyin tags without #")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument(
        "--mark-uploaded",
        nargs=2,
        metavar=("VIDEO_ID", "PLATFORM"),
        help="Manually mark uploaded: platform=douyin",
    )
    ...
    if args.mark_uploaded:
        ...
        if platform != "douyin":
            print("PLATFORM 只能是 douyin", file=sys.stderr)
            return 2
        entry = state.setdefault("items", {}).setdefault(
            video_id,
            {"source_title": "", "url": "", "douyin": None, "copy": None},
        )
